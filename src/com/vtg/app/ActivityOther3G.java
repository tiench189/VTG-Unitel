...
public class ActivityOther3G extends FragmentActivity implements CommonDefine {
	...
	private void activityCreate() {
		setContentView(R.layout.activity_other_3g);
		initView();
		initBanner();
	}
	...
	private class AsyncTaskBalanceInfo extends
		...
		protected String doInBackground(String... p) {
			...
			params.add(new ModelParam("isdn", NUMBER_HEADER
					+ preferences.getString(PreferenceKey.PHONE_NUMBER, "")));
			params.add(new ModelParam("listId", "4500,4561,4560"));
			...
			for (int i = 0; i < 10; i++) {
				if (!XMLParser.getElementValue(listName.item(i)).trim()
					...
					ModelBalance balance = new ModelBalance(
							...
							XMLParser.getElementValue(listValue.item(i)));
